...
def year_quarter(date_text: str) -> tuple[str, str]:
    try:
        ...
        return str(parsed.year), str((parsed.month - 1) // 3 + 1)
    except Exception:
        return "", ""
...
def to_gastos(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        ...
        model_base = row.get("base_imponible") if row.get("modelo_303") == "Sí" else None
        accounting_concept = row.get("tipo_gasto", "") or row.get("concepto", "")
        records.append({
            "Año": year,
            "Trimestre": quarter,
            "Tipo": row.get("tipo_documento", ""),
            "Fecha factura": row.get("fecha_factura", ""),
            ...
            "Total": row.get("total"),
            "% deducción": row.get("porcentaje_deduccion", ""),
            "Importe deducible IRPF": row.get("importe_deducible_irpf"),
